...
class Config:
    MODEL_PATH = r"/media/jun/ubuntu/bert/tohoku/bert-base-japanese-char"   
    DATA_PATH = r"bert_text.xlsx"
    RESULT_PATH = r"/tohokuresult/"
    NUM_LABELS = 2  
    MAX_LENGTH = 512
    BATCH_SIZE = 4
    NUM_EPOCHS = 5
    SEED = 42
    CLASS_NAMES = {0: "レベル1", 1: "レベル2"} 
    TEST_RATIO = 0.2
...
def main():
    ...
    try:
        ...
        best_model_path = os.path.join(Config.RESULT_PATH, "best_model_tohoku_base")
        trainer.save_model(best_model_path)
        print(f"\n最佳模型已保存至：{best_model_path}")
        
    except Exception as e:
        print(f"\n训练异常：{str(e)}")
        emergency_path = os.path.join(Config.RESULT_PATH, "emergency_model_tohoku")
    ...
    plot_confusion_matrix(labels, preds, 
                         save_path=os.path.join(Config.RESULT_PATH, "confusion_matrix_tohoku.png"))
    print("混淆矩阵已保存")
    ...
    report_path = os.path.join(Config.RESULT_PATH, "classification_report_tohoku.csv")
