...
class Config:
    MODEL_PATH = r"NICT_BERT-base_JapaneseWikipedia_32K_BPE"
    DATA_PATH = r"bert_text.xlsx"
    RESULT_PATH = r"nictresult/"
    NUM_LABELS = 2
    MAX_LENGTH = 512
    BATCH_SIZE = 4
    NUM_EPOCHS = 5
...
def main():
    ...
    try:
        ...
        best_model_path = os.path.join(Config.RESULT_PATH, "best_model_NICT_32K")
        trainer.save_model(best_model_path)
        print(f"\n最佳模型已保存至：{best_model_path}")
        
    except Exception as e:
        print(f"\n训练异常：{str(e)}")
        emergency_path = os.path.join(Config.RESULT_PATH, "emergency_model_NICT")
    ...
    plot_confusion_matrix(labels, preds, 
                         save_path=os.path.join(Config.RESULT_PATH, "confusion_matrix_NICT.png"))
    print("混淆矩阵已保存")
    ...
    report_path = os.path.join(Config.RESULT_PATH, "classification_report_NICT.csv")
